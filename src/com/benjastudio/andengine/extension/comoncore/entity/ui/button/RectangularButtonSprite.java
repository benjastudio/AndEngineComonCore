...
public class RectangularButtonSprite<T extends IManagerBundle<?, ?, ?, ?, ?, ?, ?>, TSceneType extends Enum<TSceneType>>
		extends CustomButtonSprite<T, TSceneType> {

	public RectangularButtonSprite(float pX, float pY, ITextureRegion pIcon,
			IBaseScene<T, TSceneType> pScene, boolean pMove, boolean pSound,
			OnClickListener pOnClickListener) {
		super(
				pX,
				pY,
				pScene.mBundle.getResourceManager().comon.buttonRectangular_TTR,
				pScene, pMove, pSound, pOnClickListener);
		if (pIcon != null)
			addIcon(new Sprite(0, 0, pIcon, pScene.eBundle.vbom));
	}

	public RectangularButtonSprite(float pX, float pY, String text,
			IBaseScene<T, TSceneType> pScene, boolean pMove, boolean pSound,
			OnClickListener pOnClickListener) {
		super(
				pX,
